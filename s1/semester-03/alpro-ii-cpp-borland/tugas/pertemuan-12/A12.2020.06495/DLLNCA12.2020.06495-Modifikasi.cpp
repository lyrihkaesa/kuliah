    ...
    if(head == NULL){
        ...
        while (bantu != NULL){
            ...
            cout << setiosflags(ios::left)
                ...
                << setw(margin+4) << bantu->tinggi;
            bantu = bantu->next;
            amountDatas = amountDatas + 1;
        };
    }
    
    int vLineDown2 = stringLeft+lengthName+4+4+2+margin+margin+margin;
    gotoxy(stringLeft, yInputSelect+6); 
    cout << setiosflags(ios::left)
        << setw(margin+2) << "No"
        << setw(margin+lengthName) << "Nama"
        << setw(margin+4) << "Umur"
        << setw(margin+4) << "Tinggi";
    if(amountDatas == 0){
...
void tableInfo1(){
    tableInfo(vLineRight1, hLineRight4+2, hLineRight3-(margin*2)-1, "Posisi sisip harus [2] -", margin, amountDatas-1);
    getch();
}
